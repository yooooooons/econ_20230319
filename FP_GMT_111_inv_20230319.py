...
invest_ratio = 0.2   # 보유 금액의 최대 몇 % 를 투자할것인가 (예> 0.1 <-- 보유금액 10% 투자) 
...
ma_duration_short = 4   # 단기 ma 산출 기간
ma_duration_mid = 30   # 중기 ma 산출 기간
ma_duration_long = 30   # 장기 ma 산출 기간

### buy_transaction 조건값 설정
ma_short_under_duration = 1   # 최근 몇개 기간동안의 ma_short의추이를 살펴볼것인지 지정
...
ratio_ema_long = 1.0   # 현재 구간에서의 ema_long값과 직전구간 ema_long값의 비율이 얼마 이상일때 buy transaction을 수행하는가 (여러 필요 조건중의 하나)
...
sell_force_loss = 0.07  # 강제 판매 손실율
ratio_sell_ema_mid_chg_state = 0.9997   # 권장 판매조건이 만족 안된 상태에서, ema_long 비율이 어느정도 미만이 되면 자동 판매 되게끔 (매수시의 상승추세가 꺾였다고 판단)
